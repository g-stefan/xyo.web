...
// PostgreSQL schema-upgrade primitives:
//   storageRemoveField  (PostgreSQL drops dependent indexes with the column)
//   storageRenameField  (ALTER ... RENAME COLUMN)
//   storageUpdateField  (ALTER ... ALTER COLUMN TYPE ... USING + SET DEFAULT)
//   storageUpdateTable  (descriptor -> live schema: ADD/ALTER/DROP + indexes)
...
// ---------------------------------------------------------------------------
t_section("storageUpdateField retypes a column (ALTER COLUMN TYPE ... USING)");
$connection = mock_connection();
...
check("score is integer before update", db_column_data_type($connection, "user", "score") === "integer");
check("storageUpdateField('score') succeeds", $userV2->storageUpdateField("score") === true);
check("score is bigint after update", db_column_data_type($connection, "user", "score") === "bigint");
check_eq("score value preserved through retype", "42", (string) db_first_value($connection, "user", "score"));
...
check("'score' retyped to bigint", db_column_data_type($connection, "user", "score") === "bigint");
...
// V1 indexes "email"; V2 indexes "name". The PostgreSQL driver names indexes
// after the field.
