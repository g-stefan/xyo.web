...
namespace XYO\Web\DataSource\Types\SQLite {
    ...
    class Table
    {
        ...
        function tryLoadCode($query)
        {
            ...
            if ($this->result) {
                $fields = $this->result->fetchArray(SQLITE3_ASSOC);
                if ($fields) {
                    $this->table->empty();
        ...
        {
            if ($this->result) {
                $this->nextFields = $this->result->fetchArray(SQLITE3_ASSOC);
                if ($this->nextFields) {
                    return true;
                }
            }
        ...
        {
            $query = "SELECT [name] FROM [sqlite_master] WHERE [name]=\"" . $this->name . "\";";
            $result = $this->connection->queryValue($query);
            if ($result) {
        ...
        public function createStorageIndex()
        {
            ...
            foreach ($this->info->indexes as $index) {
                $query = "CREATE INDEX IF NOT EXISTS [" . $this->name . "_" . $index . "] ON [" . $this->name . "] ([" . $index . "])";
                $result = $this->connection->query($query);
                if ($result) {
                    continue;
                }
